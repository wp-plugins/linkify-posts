...
class c2c_LinkifyPostsWidget extends WP_Widget {
	...
	private $textdomain = 'linkify-posts';
	private $title = '';
	private $description = '';
	private $config = array();
	private $defaults = array();

	public function c2c_LinkifyPostsWidget() {
		$this->title = __( 'Linkify Posts', $this->textdomain );
		...
		$this->config = array(
			...
			'title' => array( 'input' => 'text', 'default' => __( 'Posts', $this->textdomain ),
					'label' => __( 'Title', $this->textdomain ) ),
			'posts' => array( 'input' => 'text', 'default' => '',
					'label' => __( 'Posts', $this->textdomain ),
					'help' => __( 'A single post ID/slug, or multiple post IDs/slugs defined via a comma-separated and/or space-separated string.', $this->textdomain ) ),
			'before' => array( 'input' => 'text', 'default' => '',
					'label' => __( 'Before text', $this->textdomain ),
					'help' => __( 'Text to display before all posts.', $this->textdomain ) ),
			'after' => array( 'input' => 'text', 'default' => '',
					'label' => __( 'After text', $this->textdomain ),
					'help' => __( 'Text to display after all posts.', $this->textdomain ) ),
			'between' =>  array( 'input' => 'text', 'default' => ', ',
					'label' => __( 'Between posts', $this->textdomain ),
					'help' => __( 'Text to appear between posts.', $this->textdomain ) ),
			'before_last' =>  array( 'input' => 'text', 'default' => '',
					'label' => __( 'Before last post', $this->textdomain ),
			...
			'none' =>  array( 'input' => 'text', 'default' => '',
					...
					'help' => __( 'Text to appear when no posts have been found.  If blank, then the entire function doesn\'t display anything.', $this->textdomain ) )
		);

		foreach ( $this->config as $key => $value )
			$this->defaults[$key] = $value['default'];
		$widget_ops = array( 'classname' => 'widget_' . $this->widget_id, 'description' => __( 'Converts a list of posts (by slug or ID) into links to those posts.', $this->textdomain ) );
...
add_action( 'widgets_init', create_function('', 'register_widget(\'c2c_LinkifyPostsWidget\');') );
